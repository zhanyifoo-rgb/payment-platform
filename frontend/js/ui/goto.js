...
export function goToTopup(step, state, elements) {
    ...
    if (step === 1) {
        ...
        document.getElementById("reviewAmount").textContent = fmt(state.amount);
        document.getElementById("reviewTotal").textContent = fmt(state.amount);
        btnNext.textContent = "Top Up RM " + state.amount.toFixed(2);
        btnNext.disabled = false;
    }
}
...
export function refreshNextStateTopup(btnNext, state) {
    if (state.step === 1) {
        btnNext.disabled = !(state.amount > 0);
    }
}
